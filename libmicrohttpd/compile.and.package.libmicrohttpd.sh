...
PROG_NAME="libmicrohttpd-dev"
PROG_VERSION="0.9.72"
ARCHITECTURE="arm64"
PKG_DESTINATION_PATH="$HOME/debpkgs/${PROG_NAME}_${PROG_VERSION}_${ARCHITECTURE}"
PROG_EXTERNAL_LOCATION="http://ftpmirror.gnu.org/libmicrohttpd/libmicrohttpd-$PROG_VERSION.tar.gz"
PROG_DEPENDS="libc6"
PROG_DESCRIPTION="A small C library that is supposed to make it easy to run an HTTP server as part of another application."
PRE_INSTALL="no"
PRE_INSTALL_INSTRUCTIONS=""
POST_INSTALL="yes"
POST_INSTALL_INSTRUCTIONS="sudo ldconfig"
wget $PROG_EXTERNAL_LOCATION
tar xvf libmicrohttpd-$PROG_VERSION.tar.gz
cd libmicrohttpd-$PROG_VERSION
